/**
 * Do‘kon (ERP) zaxirasi — `erp_store_inventory` (CRM `product_inventory` emas).
 */
export function mergeErpStoreInventoryRow(product) {
  ...
  if (row && typeof row === 'object') {
    ...
    next.avg_cost_usd = row.avg_cost_usd ?? 0
    next.stock_value_usd = row.stock_value_usd ?? 0
    next.inventory_status = row.status ?? null
  } else {
    next.stock = 0
    next.stock_by_color = null
    next.avg_cost_usd = 0
    next.stock_value_usd = 0
